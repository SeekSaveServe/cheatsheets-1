...
\begin{multicols}{2}
    ...
    \begin{itemize}
      ...
      \begin{itemize}
        ...
        \item Cost: $2|\pi^*_L(R)|(\log_m(N_0)+1)$ + $2|\pi^*_L(S)|(\log_m(N_0)+1)$ + $|\pi^*_L(R)|+|\pi^*_L(S)|$
        \item merging cost is $|R|$ + $||R||*|S|$ if each tuple of R requires a full scan of S
        \item Optimisation: $B \ge N(R,i) + N(S,i)$
        \item Cost: cost of getting R, S + write out ($|R|+|S|$) + merge ($|R|+|S|$)
      \end{itemize}

      \item \textbf{Grace Hash Join} \newline
      \begin{itemize}
        \item Partition into $B-1$ partitions
        \item If no partition overflow:
        \item Cost to partition R, S + Cost of probe phase
...
\end{itemize}
% Lecture 7
\section{L7: Transaction Management}
...
\begin{itemize}
  ...
  \item \textbf{Conflict Serializable} Conflict equivalent to serial schedule
...
\begin{itemize}
  \item \textbf{WW} from T1 to T2: T1 writes some version of X and T2 writes the immediate successor
  \item \textbf{WR} from T1 to T2: T1 writes some version of X which is read by T2
  \item \textbf{RW} from T1 to T2: T1 reads some version of X and T2 writes the immediate successor
