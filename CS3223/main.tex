...
\begin{itemize}
  \item \textbf{Dirty Read-WR} T2 read uncommited write from T1
  \item \textbf{Unrepeatable Read-WR} T2 updates an object that T1 has previously read and T2 commits while T1 is still in progress $\rightarrow$ T1 can get a different value from read
  \item \textbf{Lost Update-WW} T2 overwrites the value of an object that has been modified by T1 while T1 is still in progress
...
\begin{itemize} 
  ...
  \item \textbf{Cascadeless} Whenever $T_i$ reads from $T_j$ in S, Commit must precede this action. This is desirable, not compulsory.
