...
\begin{enumerate}
  ...
  \item Max-Clique \\
  \textbf{Problme} Given an undirected graph G = (V,E), a subset $X \subseteq V$ is said to be a clique if for each pair of vertices $u,v \in X$, $(u,v) \in E_X$. The maximum clique is the largest clique in the graph.
  \item Hamiltonian Cycle (both directed and undirected)
  \item Hamiltonian Path (undirected)
  \item Traveling Sales Person \\ 
  \textbf{Problem} Given a complete graph G = (V,E), find a Hamiltonian cycle of minimum weight.
  \item Subset Sum \\ 
